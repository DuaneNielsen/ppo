...
def setup(logdir='log', logfile='log', console_minlevel=logging.DEBUG, file_minlevel=logging.DEBUG):
    ...
    logfile = '{logfile}-{year:04d}{mon:02d}{day:02d}-' \
              '{hour:02d}{min:02d}{sec:02d}.log'.format(
        logfile=logfile, year=t.year, mon=t.month, day=t.day,
    ...
    filehandler = logging.handlers.RotatingFileHandler(
        ...
        backupCount=100)
    filehandler.setLevel(file_minlevel)
    ...
    streamhandler.setLevel(console_minlevel)
    streamhandler.addFilter(InfoFilter())
    streamhandler.setFormatter(logging.Formatter('%(levelname)s: %(message)s'))
    logger.addHandler(streamhandler)

    # Set up logging to the console stderr.
